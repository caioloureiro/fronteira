...
$check_duplicate = $conn->query("SELECT id FROM administracao WHERE titulo = '". $conn->real_escape_string($titulo) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Criou a página de administração ". $titulo ."','". date( 'Y-m-d H:i:s' ) ."');";
		...
		<div class="box">
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<a href="'.$raiz_admin .'matriz?pagina=administracao" ><div class="linha"><button>Retornar</button></div></a>
					';
