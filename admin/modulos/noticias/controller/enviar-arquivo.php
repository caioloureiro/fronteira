...
function redimensionarImagem($caminhoOrigem, $caminhoDestino, $novaLargura) {
    ...
    switch ($tipo) {
        case IMAGETYPE_JPEG:
            imagejpeg($imagemRedimensionada, $caminhoDestino, 90); // Qualidade 90
            break;
        case IMAGETYPE_PNG:
...
if( $arquivo_subir_array['error'] == 0 ){ //SEM ERROS
	...
	$caminhoDestino = $pasta . $nome_final;

	move_uploaded_file( $arquivo_subir_array["tmp_name"], $caminhoDestino ); // Salva temporariamente

	/* Redimensionar a imagem para 800px de largura */
	redimensionarImagem($caminhoDestino, $caminhoDestino, 800);
}
/*End - SUBIR ARQUIVO*/
