...
$check_duplicate = $conn->query("SELECT id FROM diario_oficial WHERE titulo = '". $conn->real_escape_string($titulo) ."' LIMIT 1");
...
$sql = "INSERT INTO diario_oficial (
	titulo,
	numero,
	data,
	arquivo
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: diario_oficial. Criou o item ". $titulo ."','". date( 'Y-m-d H:i:s' ) ."');";
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<script>
						...
						window.location.href = "'. $raiz_admin .'matriz?pagina=diario_oficial";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=diario_oficial" ><div class="linha"><button>Retornar</button></div></a>
					';
