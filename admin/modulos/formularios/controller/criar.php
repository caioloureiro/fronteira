...
$prefixo_protocolo = $_POST['prefixo_protocolo'];
...
$check_duplicate = $conn->query("SELECT id FROM formularios WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: formularios. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
		...
		<div class="box">
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<a href="'. $raiz_admin .'matriz?pagina=formularios" ><div class="linha"><button>Retornar</button></div></a>
					';
