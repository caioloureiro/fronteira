...
$link = $_POST['link'];
$target = $_POST['target'];
$pagina_alvo = $_POST['pagina_alvo'];
$subpagina_alvo = $_POST['subpagina_alvo'];
...
$check_duplicate = $conn->query("SELECT id FROM menu_interno WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: menu_interno. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
		...
		<div class="box">
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<a href="'. $raiz_admin .'matriz?pagina=menu_interno" ><div class="linha"><button>Retornar</button></div></a>
					';
