...
$check_duplicate = $conn->query("SELECT id FROM licitacoes_situacao WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql = "INSERT INTO licitacoes_situacao (
	nome, 
	created_at, 
	updated_at
) VALUES (".
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: licitacoes_situacao. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<script>
						...
						window.location.href = "'. $raiz_admin .'matriz?pagina=licitacoes_situacao";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=licitacoes_situacao" ><div class="linha"><button>Retornar</button></div></a>
					';
