	...
	<body>
		...
		<div class="lightbox formularios_respostas-visualizar on">
			...
			<div class="linha linha-auto">
				<div class="comentario">Para visualizar os arquivos <strong><a href="<?php echo $raiz_admin ?>matriz?pagina=pasta-formularios_arquivos" target="_blank">clique aqui para acessar a pasta formularios_arquivos em uma nova guia</a></strong> e buscar o arquivo desejado.</div>
			</div>
					...
					foreach( $formularios_respostas_array as $resp ){
						...
						if( $resp['id'] == $_GET['id'] ){
							...
							foreach( $respostas_array as $respItem ){
								...
								if( $respItem_array[0] == 'formulario_id' ){ $formulario_id = $respItem_array[1]; }
		...
		<script>
			...
			function voltar(){
				...
				window.location.href = 'visualizar?formulario_id=<?php echo $formulario_id ?>';
