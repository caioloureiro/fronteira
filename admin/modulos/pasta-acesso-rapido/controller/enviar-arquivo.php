				...
				if( isset( $_POST ) and $_SERVER['REQUEST_METHOD'] == "POST" ){
					...
					foreach( $_FILES['enviarArquivoItem']['name'] as $f => $name ){
						...
						if($_FILES['enviarArquivoItem']['error'][$f] == 0){
							...
							$sql = "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('".$_COOKIE['fronteira_ADMIN_SESSION_usuario'] ."','acesso-rapido: Subiu o arquivo: ".$pasta.$nome_final."','". date( 'Y-m-d H:i:s' ) ."');";
							$conn->multi_query( $sql );
							$conn->close();
							
							//NÃO VOU RENOMEAR AGORA:
							//echo'<script> window.location = "../view/novo-02?arquivo='. $nome_final .'"; </script>';
							echo'<script> window.location = "'. $raiz_admin .'matriz?pagina=pasta-acesso-rapido"; </script>';
