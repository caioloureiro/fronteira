...
$check_duplicate = $conn->query("SELECT id FROM esic WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql = "INSERT INTO esic (
	...
	"'". $status ."',".
	"'". $identificacao ."',".
	"'". ($data ? date('Y-m-d H:i:s', strtotime($data)) : NULL) ."',".
	"'". $mensagem ."',".
	"'". $resposta ."',".
	"'". $anexo ."'".
");";

$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: esic. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<script>
						...
						window.location.href = "'. $raiz_admin .'matriz?pagina=esic";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=esic" ><div class="linha"><button>Retornar</button></div></a>
					';
