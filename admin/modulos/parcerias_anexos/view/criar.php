	...
	<body>
		...
		<div class="lightbox anexo-nova on">
			...
			<form action="../controller/criar.php" method="POST">
				...
				<div class="linha">
					...
					<div class="col90">
						<span>
							<select class="parceria" name="parceria">
								<option value="0">Selecione o parceria</option>
								
								<?php
									
									foreach( $parcerias_array as $conc ){
										
										echo'<option value="'. $conc['id'] .'">'. $conc['numero'] .'</option>';
		...
		<script>
			...
			function voltar(){
				...
				window.location.href = '<?php echo $raiz_admin ?>matriz?pagina=parcerias_anexos';
				...
				if( document.querySelector('[name="arquivo_subir"]').files[0] ){
					...
					xhr.onreadystatechange = function(){
						...
						if( 
							...
							document.querySelector('.item-escolher-arquivo-input').value = xhr.responseText;
