...
$tipo = $arquivo_array[ count( $arquivo_array ) - 1 ];
//dd( $tipo );


// PROTEÇÃO: Verificar se já existe (evitar duplicatas por submit múltiplo)
$check_duplicate = $conn->query("SELECT id FROM downloads WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: downloads. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
		...
		<div class="box">
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<a href="'. $raiz_admin .'matriz?pagina=downloads" ><div class="linha"><button>Retornar</button></div></a>
					';
