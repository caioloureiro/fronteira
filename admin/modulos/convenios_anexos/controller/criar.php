...
$check_duplicate = $conn->query("SELECT id FROM convenios_anexos WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql = "INSERT INTO convenios_anexos (
	nome, 
	arquivo, 
	convenio
) VALUES (".
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: convenios_anexos. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<script>
						...
						window.location.href = "'. $raiz_admin .'matriz?pagina=convenios_anexos";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=convenios_anexos" ><div class="linha"><button>Retornar</button></div></a>
					';
