<div class="lightbox item-imagens" style="z-index:99999">
	...
	<div class="escolher-imagem-cards-box">
		...
		<?php
			...
			if( isset( $_GET['id'] ) ){ $galeria_id = $_GET['id']; }
			...
			for($i = 0;$i <= $filecount;$i++){
				...
				if(
					...
					<div
						onClick="
							incluirImagem( `'.$arquivo[$i].'`, `'. $galeria_id .'` );
...
function incluirImagem( imagem, galeria_id ){

	//console.log( 'imagem', imagem ); 

	var formData_incluirImagem = new FormData();
	formData_incluirImagem.append( 'imagem', imagem );
	formData_incluirImagem.append( 'galeria_id', galeria_id );
	...
	'<div class="thumb thumb_'+ exibir_incluirImagem_id +'">'+
		...
		'<div '+
			'class="thumb-btn"'+
			'onclick="escolherCapa( `thumb_'+ exibir_incluirImagem_id +'`, `'+ imagem +'` )"'+
		'>Escolher</div>'+
	'</div>'+
	'';
