...
if( $arquivo_aceito == 0 ){
	...
	echo'<script> alert("O tipo de arquivo '. $tipo_de_arquivo[1] .' não é aceito."); </script>';
	return; 
...
if( $arquivo_subir_array['error'] == 4 ) {
	...
	echo'<script> alert("O arquivo possui erro: '. $arquivo_subir_array['error'] .'"); </script>';
	return; 
...
if( $arquivo_subir_array['error'] == 0 ){ //SEM ERROS
	...
	if( $arquivo_subir_array['size'] > $tamanho_maximo ) {
		...
		echo'<script> alert("O arquivo é muito grande. Deve ser menor que 2MB."); </script>';
		return;
...
//echo current( $_POST );
$galeria_id = current( $_POST );

$sql .= "INSERT INTO galeria_imagens (
	nome, 
	data, 
	imagem, 
	galeria_id 
