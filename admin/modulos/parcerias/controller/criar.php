...
$check_duplicate = $conn->query("SELECT id FROM parcerias WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: parceria. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
		...
		<div class="box">
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					echo'
					<script>
						alert("Parceria CRIADO com sucesso!"); 
						window.location.href = "'. $raiz_admin .'matriz?pagina=parcerias";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=parcerias" ><div class="linha"><button>Retornar</button></div></a>
					';
