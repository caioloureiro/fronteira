			...
			<?php
				...
				if( isset( $_POST ) and $_SERVER['REQUEST_METHOD'] == "POST" ){
					...
					foreach( $_FILES['enviarArquivoItem']['name'] as $f => $name ){
						...
						$tipo_de_arquivo_array = explode( '.', trim( strip_tags( $_FILES['enviarArquivoItem']['name'][$f] ) ) );
						...
						if($_FILES['enviarArquivoItem']['error'][$f] == 0){
							...
							if( $_FILES['enviarArquivoItem']['size'][$f] > $tamanho_maximo ) {
								...
								echo'
								<script> 
									alert("Arquivo muito grande para enviar para o servidor.");
									window.history.back(); 
								</script>
								';
							...
							$sql = "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('".$_COOKIE['fronteira_ADMIN_SESSION_usuario'] ."','arquivos: Subiu o arquivo: ".$pasta.$nome_final."','". date( 'Y-m-d H:i:s' ) ."');";
							$conn->multi_query( $sql );
							$conn->close();
							
							//NÃO VOU RENOMEAR AGORA:
							//echo'<script> window.location = "../view/novo-02?arquivo='. $nome_final .'"; </script>';
							echo'<script> window.location = "'. $raiz_admin .'matriz?pagina=pasta-arquivos"; </script>';
