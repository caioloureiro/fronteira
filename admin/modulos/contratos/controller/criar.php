...
$valor = isset($_POST['valor']) ? str_replace(',', '.', str_replace('.', '', $_POST['valor'])) : null;
...
// Construir a query SQL com todos os campos

// PROTEÇÃO: Verificar se já existe (evitar duplicatas por submit múltiplo)
$check_duplicate = $conn->query("SELECT id FROM contratos WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: contrato. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<script>
						...
						window.location.href = "'. $raiz_admin .'matriz?pagina=contratos";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=contratos" ><div class="linha"><button>Retornar</button></div></a>
					';
