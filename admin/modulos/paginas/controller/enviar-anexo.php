...
if( move_uploaded_file( $arquivo_subir_array["tmp_name"], $pasta.$nome_final ) ){ //SOBE O ARQUIVO
	...
	$pagina = isset($_POST['pagina']) ? $_POST['pagina'] : 'nova';
	...
	// Se for uma página nova, não salvar no banco ainda (será salvo quando criar a página)
	if($pagina != 'nova') {
		...
		$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Subiu anexo para página: ".$pasta.$nome_final."','". date( 'Y-m-d H:i:s' ) ."');";
