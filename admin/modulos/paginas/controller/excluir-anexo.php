...
/*Start - EXCLUIR ANEXO*/
$anexo_id = isset($_POST['id']) ? intval($_POST['id']) : 0;
...
$sql_buscar = "SELECT * FROM paginas_anexos WHERE id = $anexo_id";
...
$sql_desativar = "UPDATE paginas_anexos SET ativo = 0, updated_at = '$hoje' WHERE id = $anexo_id";

if( $conn->query($sql_desativar) ){
	
	// Log da ação
	$sql_log = "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Desativou anexo de página: ".$anexo['arquivo']."','". date( 'Y-m-d H:i:s' ) ."')";
	$conn->query($sql_log);
