...
// Se o arquivo foi enviado como array (arquivos_anexos[]), pegar o primeiro elemento
if(isset($arquivo_subir_array['name']) && is_array($arquivo_subir_array['name'])) {
	$arquivo_subir_array = [
		'name' => $arquivo_subir_array['name'][0],
		'type' => $arquivo_subir_array['type'][0],
		'tmp_name' => $arquivo_subir_array['tmp_name'][0],
		'error' => $arquivo_subir_array['error'][0],
		'size' => $arquivo_subir_array['size'][0]
	];
}
...
if( move_uploaded_file( $arquivo_subir_array["tmp_name"], $pasta.$nome_final ) ){ //SOBE O ARQUIVO
	...
	$legislacao = isset($_POST['legislacao']) ? $_POST['legislacao'] : 'nova';
	...
	// Se for uma legislação nova, não salvar no banco ainda (será salvo quando criar a legislação)
	if($legislacao != 'nova') {
		$sql .= "INSERT INTO legislacoes_anexos (
			ativo,
			created_at,
			updated_at,
			nome, 
		...
		$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Subiu anexo para legislação: ".$pasta.$nome_final."','". date( 'Y-m-d H:i:s' ) ."');";
