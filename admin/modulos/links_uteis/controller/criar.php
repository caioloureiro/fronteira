...
$check_duplicate = $conn->query("SELECT id FROM links_uteis WHERE nome = '". $conn->real_escape_string($nome) ."' LIMIT 1");
...
$sql .= "INSERT INTO rastrear_usuario (usuario, descricao, horario) VALUES ('". $_COOKIE['fronteira_ADMIN_SESSION_usuario'] ." - ". $_SERVER['REMOTE_ADDR'] ."','Tabela: links_uteis. Criou o item ". $nome ."','". date( 'Y-m-d H:i:s' ) ."');";
			...
			<?php
				...
				if ( $conn->multi_query( $sql ) === TRUE ) {
					...
					<script>
						...
						window.location.href = "'. $raiz_admin .'matriz?pagina=links_uteis";
					</script>
					';
					...
					<a href="'. $raiz_admin .'matriz?pagina=links_uteis" ><div class="linha"><button>Retornar</button></div></a>
					';
